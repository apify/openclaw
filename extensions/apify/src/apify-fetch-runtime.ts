...
export type CrawlerType = "cheerio" | "jsdom" | "playwright:firefox" | "playwright:chrome";
...
function missingApifyFetchKeyPayload() {
  return {
    error: "missing_apify_api_key",
    message:
      "web_fetch (apify) needs an Apify API token. Set APIFY_API_KEY in the environment, or configure plugins.entries.apify.config.webFetch.apiKey.",
    docs: "https://apify.com/apify/website-content-crawler",
  };
}
...
export async function executeApifyFetch(
  ...
  if (!apiKey) {
    return missingApifyFetchKeyPayload();
  }
  ...
  if (!item) {
    return {
      error: "no_content",
      message: "Apify Website Content Crawler returned no content for the URL.",
      url,
    };
  }
