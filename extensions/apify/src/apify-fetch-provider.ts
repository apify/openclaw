...
const ApifyFetchSchema = {
  ...
  properties: {
    ...
    crawlerType: {
      type: "string",
      enum: ["cheerio", "jsdom", "playwright:firefox", "playwright:chrome"],
      description:
        'Crawler type. "cheerio" (default) is fastest. "playwright:firefox" handles JS-heavy and bot-protected pages.',
    },
    maxChars: {
      type: "number",
  ...
  return {
    ...
    createTool: ({ config }) => {
      ...
      const timeoutSeconds = resolveTimeoutSeconds(
        webFetchConfig?.timeoutSeconds,
        DEFAULT_TIMEOUT_SECONDS,
      );
      return {
        description:
          "Fetch a web page using Apify Website Content Crawler. Returns full page content as markdown. " +
          'Use crawlerType "playwright:firefox" for JS-heavy or bot-protected pages; "cheerio" for fast plain-HTML pages.',
