...
describe("apify web fetch provider", () => {
  ...
  it("uses low memory for cheerio/jsdom and high memory for playwright crawlers", async () => {
    vi.stubEnv("APIFY_API_KEY", "");
    const config = {
      plugins: {
        entries: {
  ...
  it("returns no_content error when the API returns an empty array", async () => {
    vi.stubEnv("APIFY_API_KEY", "");
    const mockFetch = makeFetchMock([]);
    global.fetch = mockFetch as unknown as typeof global.fetch;
    ...
    const result = (await tool.execute({ url: "https://example.com" })) as Record<string, unknown>;
    expect(result).toMatchObject({ error: "no_content" });
  });
